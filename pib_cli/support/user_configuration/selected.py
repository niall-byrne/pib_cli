...
from pib_cli.support import dev_container


class SelectedUserConfigurationEntry:
  """A parsed user configuration entry, ready to use.

  :param user_configuration: A single loaded user configuration.
  """

  def __init__(
      self, user_configuration: yaml_keys.TypeUserConfiguration
  ) -> None:
    self.containerized = dev_container.DevContainer.is_container()
  ...
  def is_executable(self) -> bool:
    ...
    if not self.containerized and container_only_flag:
      return False
    return True
